...
export default function ContractorListComponent() {
  ...
  const [pages, setPages] = useState<Contractors["meta"]>({
    page: 1,
    count: 0,
    limit: 0,
    from: 0,
  ...
  useEffect(() => {
    const fetchContractors = async () => {
      setIsLoading(true);
      try {
        const response = await contractorService.getContractors(`?page=${page}`);
        setContractors(response.data);
        setPages(response.meta);
      } catch (err) {
        const errorMessage = err instanceof Error ? err.message : "Failed to load contractors";
